    ...
    <div class="sales">
        <div class="filter_tab2_con">
            <div class="filter_tab2_con_con">
                <span :class='idx.indexOf(item.typeId)>=0?"filter_key cur":"filter_key"' v-for="(item,index) in data" :key="index" @click="setokIs(item.typeId,item.name)">{{item.name}}</span>
            </div>
...
export default {
    ...
    methods:{
        ...
        ok(){
            this.salelist.sort(function(a,b){
                return a.id-b.id;
            })
            this.$emit('change',this.salelist);
        }
    },
    watch:{
